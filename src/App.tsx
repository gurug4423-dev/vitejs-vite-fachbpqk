...
function exportToExcel(tasks) {
  ...
  const G_MONTHS = ["Sep'25","Oct'25","Nov'25","Dec'25","Jan'26","Feb'26","Mar'26","Apr'26","May'26"];
  const G_STARTS = [new Date("2025-09-01"),new Date("2025-10-01"),new Date("2025-11-01"),new Date("2025-12-01"),new Date("2026-01-01"),new Date("2026-02-01"),new Date("2026-03-01"),new Date("2026-04-01"),new Date("2026-05-01")];
  const G_END = new Date("2026-05-31");
  const ganttHeader = ["#","Project Title","Vendor","Status","Progress %",...G_MONTHS,"Start Date","End Date"];
  const ganttData = [
  ...
  tasks.forEach(t => {
    ...
    G_MONTHS.forEach((_,i) => {
      const mStart = G_STARTS[i];
      const mEnd = i < G_MONTHS.length-1 ? G_STARTS[i+1] : G_END;
      if (startD && endD && startD < mEnd && endD >= mStart) {
        row.push(st === "Completed" ? "██ Done" : st === "Pending" ? "▒▒ Pending" : st === "In Progress" ? "░░ Active" : "── Planned");
      } else {
        row.push("");
      }
    });
  ...
  ws4["!cols"] = [{wch:4},{wch:32},{wch:22},{wch:14},{wch:12},...G_MONTHS.map(()=>({wch:11})),{wch:14},{wch:14}];
